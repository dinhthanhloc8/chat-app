...
function ConversationCard({ data }) {
    ...
    return (
        <div
            className={`flex min-h-[100px] justify-between items-center p-[16px_10px] cursor-pointer border-b-[#ffffff1f] rounded-2xl w-full  hover:bg-[var(--white-2)]
                ${active ? 'bg-[var(--white-2)]' : ''}`}
            onClick={() => {
                navigate(`/c/${data.id}`);
                setRead(true);
        ...
        >
            ...
            <div className="w-[80%] flex flex-col justify-around">
                <div className="flex items-center justify-between">
                    <h1
                        className={`truncate text-[1rem] font-bold w-[70%] text-[var(--black)]`}
                    >
                        {name}
                    </h1>
                </div>
                {lastMessage && (
                    <div className="flex items-center ">
                        <p
                            className={`text-sm text-[#5c5b5bfd] truncate max-w-[50%]`}
                        >
                            {lastMessage}
                        </p>
                        <FiberManualRecordIcon
                            fontSize="small"
                            className={`text-[#5c5b5bfd]`}
                            sx={{
                                margin: '0 6px',
                                height: '8px',
                                width: '8px',
                        ...
                        />
                        <p className={`text-[0.7rem] text-[#5c5b5bfd]`}>
                            {time}
                        </p>
                    </div>
                )}
            ...
            {!read && (
                <div>
                    <CircleIcon className="text-[#2598d2] !text-[0.8rem]" />
                </div>
            )}
        </div>
    );
